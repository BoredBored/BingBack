...
def makeFolder(name):
    ...
    if name == '':
        if SeeDebugMessages: print('ERROR: Please specify a folder name -- ""/NULL is not a folder name')
        return 2
    elif isdir(name) == False:
        if SeeDebugMessages: print("Creating folder \'%s\'" % name)
        mkdir(name)
        return 0
    else:
        if SeeDebugMessages: print("Folder \'%s\' already exists" % name)
        return 1
...
def downloadPic(url, name):
    if str == '':
        ...
        if SeeDebugMessages: print("File '%s' already exists" % name)
        return 1
...
def ImgPathFunc():
    ...
    req = \
        Request('http://www.bing.com/HPImageArchive.aspx?format=js&idx=0&n=1&mkt=en-US'
                )
    ImgJSON = ''
    res = ''
    try:
...
def GETName(picPath):
    ...
    if ('/th?id=' in picPath) is False:
        print("String \'/th?id=' not in string \'%s\'"
            % picPath)
    else:
        return re.search(".*\/th\?id=([^&]+).*", picPath, re.IGNORECASE).group(1)
...
def change_wallpaper_win(SPISETDESKWALLPAPER, WALLPAPER_PATH):
    ...
    sys_parameters_info = \
        (ctypes.windll.user32.SystemParametersInfoW if struct.calcsize('P'
         ) * 8 == 64 else ctypes.windll.user32.SystemParametersInfoA)
...
def set_wallpaper_linux(file_loc, debug):
    ...
    from subprocess import Popen as ex
    from sys import stderr
    desktop_env = distro.linux_distribution()
    if debug:
        print('Desktop_env = %s' % str(desktop_env))
        print(file_loc)
    try:
        uri = '"file://%s"' % file_loc
        if 'Ubuntu' in desktop_env or 'ubuntu' in desktop_env \
            or 'Gnome' in desktop_env or 'gnome' in desktop_env:
            if debug: print('Ubuntu/Gnome Detected')
            #if uri not in subprocess.run(["gsettings", "get", "org.gnome.desktop.background", "picture-uri"], capture_output=True):
            args = ['gsettings', 'set', 'org.gnome.desktop.background',
                    'picture-uri', uri]
            if debug: printArr(args)
            ex(args)
            #else:
            #	print("ERROR: File already set as backgound")
        elif 'gnome2' in destop_env:

                                         # Not tested

            if debug: print('Gnome2 detected')
            args = [
                'gconftool-2',
                '-t',
                'string',
            ...
            if debug: print('KDE3/Trinity detected')
            args = \
                'dcop kdesktop KBackgroundIface setWallpaper 0 "%s" 6' \
                % file_loc
            if debug: printArr(args)
            ex(args, shell=True)
...
def setWallpaper(path, debug):
    ...
    if isabs(pathy) == False:
        ...
        if debug: print("Detected absolute path of \'%s\' as %s" % (path, pathy))
...
def main():
    ...
    if SetDestopWallpaper == True:
        if SeeDebugMessages: print('~ Setting as wallpaper ~')
        if setWallpaper(ImgN, SeeDebugMessages) == False:
